...
def relatorio_diretoria():
    ...
    return send_file(arquivo, download_name=f'relatorio_diretoria_{pd.Timestamp.now().strftime("%Y%m%d")}.xlsx', as_attachment=True)
...
def relatorio_jogador(ogol_id):
    ...
    return send_file(arquivo, download_name=f'relatorio_{nome}_{pd.Timestamp.now().strftime("%Y%m%d")}.xlsx', as_attachment=True)
...
def relatorio_comissao_completo():
    ...
    nome = membro.iloc[0].get('nome', 'membro').replace(' ', '_')
    return send_file(arquivo, download_name=f'relatorio_completo_{nome}_{pd.Timestamp.now().strftime("%Y%m%d")}.xlsx', as_attachment=True)